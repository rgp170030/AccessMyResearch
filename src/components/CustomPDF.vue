<template>
    ...
    <div>
		...
		<select v-model="src" style="width: 30em">
			<option v-for="item in pdfList" :key="item" :value="item" v-text="item"></option>
		</select>
	...
	<div style="width: 100%" class="scroll">
		...
		<pdf v-if="show" ref="pdf" :src="src" :page="page" :rotate="rotate" @progress="loadedRatio = $event" @error="error" @num-pages="numPages = $event" @link-clicked="page = $event"></pdf>
	</div>
	</div>
</template>
<script>
